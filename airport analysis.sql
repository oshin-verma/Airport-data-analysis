...
SELECT 
    Fly_date,
    STR_TO_DATE(Fly_date, '%d/%m/%y') AS Converted_Date,
    MONTH(STR_TO_DATE(Fly_date, '%d/%m/%y')) AS Month
FROM 
    airports
LIMIT 10;

-- Find the month with peak passengers per city
SELECT 
    mpc.Origin_city,
    mpc.Month,
...
FROM 
    (
        -- Step 1: calculate monthly totals per city
        SELECT 
            Origin_city,
            MONTH(STR_TO_DATE(Fly_date, '%d/%m/%y')) AS Month,
            SUM(Passengers) AS Total_Passengers
...
-- Step 2: find max passengers per city and join
JOIN 
    (
        SELECT 
            Origin_city, 
...
ON 
    mpc.Origin_city = mp.Origin_city
